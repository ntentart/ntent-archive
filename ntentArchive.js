...
class NtentArchive {
    ...
    #lookupMimeTypes(contentOptions) {
        if (contentOptions.image) {
            contentOptions.image.mimetype = mime.lookup(contentOptions.image.filename);
        }
        if (contentOptions.gif) {
            contentOptions.gif.mimetype = mime.lookup(contentOptions.gif.filename);
        }
        if (contentOptions.video) {
            contentOptions.video.mimetype = mime.lookup(contentOptions.video.filename);
        }
        if (contentOptions.other) {
            contentOptions.other.mimetype = mime.lookup(contentOptions.other.filename);
        }
        ...
        return new Promise(async (resolve, reject) => {
            ...
            if (contentOptions.html) {
                ...
                if (this.preferredGatewayBaseUrl)
                    nftMetadata.projectGatewayUrl = "https://" + result + "." + this.preferredGatewayBaseUrl + "/index.html";
    ...
    #validateTags(tags) {
        ...
        if (this.enforceTags)
            if (tags.length < 5) {
                valid = false;
                msg += "Must include a minimum of 5 tags! This is so we can find this content later! ";
            }
    ...
    #validateNFTContent(contentOptions) {
        ...
        if (contentOptions.html) {
            ...
            if (contentOptions.video || contentOptions.other) {
                valid = false;
                msg += "Cannot have an 'html' project with 'video' or 'other' projects at the same time.";
            }
        }
    ...
    #validateFileContent(contentOptions) {
        ...
        if (contentOptions.html) {
            valid = false;
            msg += "this method is for single files, use 'archiveHtmlSite' function instead. ";
        }
